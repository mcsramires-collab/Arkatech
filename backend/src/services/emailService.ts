...
/**
 * Notificação de Embarques Retroativos (pacote de 23/09, compartilhado pelo usuário) — envia à
 * corretora nova quando uma Troca de Parceria (`POST /admin/policies/:id/trocar-parceria`) tem
 * `vigencia_inicio` no passado.
 */
export interface PartnerChangeEmailParams {
  to: string;
  nomeDestinatario: string;
...
function papelEmPortugues(papel: PartnerChangeEmailParams['papel']): string {
  if (papel === 'lider') return 'Corretora Líder';
  if (papel === 'cocorretora') return 'Cocorretora';
  return 'Assessoria';
}

function buildPartnerChangeEmailHtml(params: PartnerChangeEmailParams): string {
  const primeiroNome = (params.nomeDestinatario || '').trim().split(/\s+/)[0] || params.nomeDestinatario;
  const dataFormatada = new Date(params.dataRetroativa).toLocaleDateString('pt-BR');
  const prazoFormatado = new Date(params.prazoLimite).toLocaleString('pt-BR');
  return `<!doctype html>
<html lang="pt-BR">
  <body style="margin:0;padding:0;background-color:#f4f5f7;font-family:Arial,Helvetica,sans-serif;">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f5f7;padding:32px 0;">
        ...
        <td align="center">
          <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background-color:#ffffff;border-radius:12px;overflow:hidden;">
            <tr>
              <td style="padding:32px 32px 8px 32px;">
                <p style="margin:0;font-size:13px;letter-spacing:0.04em;text-transform:uppercase;color:#6b7280;">Arckatech</p>
                <h1 style="margin:8px 0 0 0;font-size:20px;color:#111827;">Você foi vinculado como ${papelEmPortugues(params.papel)}</h1>
              </td>
            </tr>
            <tr>
              <td style="padding:16px 32px;">
                <p style="margin:0 0 12px 0;font-size:14px;line-height:1.6;color:#374151;">Olá, ${primeiroNome},</p>
                <p style="margin:0 0 12px 0;font-size:14px;line-height:1.6;color:#374151;">
                  Sua corretora passou a ser ${papelEmPortugues(params.papel)} da apólice <strong>${params.numeroApolice}</strong>,
                  com vigência retroativa a partir de <strong>${dataFormatada}</strong>.
                  Você precisa decidir se assume a responsabilidade pelos embarques já realizados desde essa data.
                </p>
                <p style="margin:0 0 12px 0;font-size:14px;line-height:1.6;color:#374151;">
                  Prazo para responder: <strong>${prazoFormatado}</strong>. Sem resposta até lá, a notificação expira automaticamente.
                </p>
              </td>
            </tr>
            <tr>
              <td style="padding:8px 32px 24px 32px;" align="center">
                <a href="${params.respostaUrl}" style="display:inline-block;background-color:#111827;color:#ffffff;text-decoration:none;font-size:14px;font-weight:600;padding:12px 24px;border-radius:8px;">
                  Responder agora
                </a>
              </td>
            </tr>
            <tr>
              <td style="padding:0 32px 32px 32px;">
                <p style="margin:0;font-size:12px;line-height:1.6;color:#9ca3af;">
                  Se o botão não funcionar, copie e cole este link no navegador:<br />
                  <span style="word-break:break-all;">${params.respostaUrl}</span>
                </p>
              </td>
            </tr>
          </table>
 ...
 * Notificação de Embarques Retroativos (pacote de 23/09, compartilhado pelo usuário) — envia à
 * corretora nova quando uma Troca de Parceria (`POST /admin/policies/:id/trocar-parceria`) tem
 * `vigencia_inicio` no passado. Mesmo padrão de "nunca lança" de `sendActivationInviteEmail`.
 */
export async function sendPartnerChangeNotificationEmail(params: PartnerChangeEmailParams): Promise<SendEmailResult> {
  const apiKey = process.env.RESEND_API_KEY;
  const fromEmail = process.env.RESEND_FROM_EMAIL || 'convites@arckatech.com.br';

  if (!apiKey) {
    console.warn(
      `[emailService] RESEND_API_KEY não configurada — notificação de embarque retroativo para ${params.to} NÃO enviada (só logada).`
    );
    return { enviado: false, motivo: 'RESEND_API_KEY não configurada no ambiente do servidor.' };
  }
  ...
  try {
    const response = await fetch(RESEND_API_URL, {
      ...
      body: JSON.stringify({
        from: fromEmail,
        to: [params.to],
        subject: `Ação necessária: embarques retroativos da apólice ${params.numeroApolice}`,
        html: buildPartnerChangeEmailHtml(params)
      })
    });

    if (!response.ok) {
      const errBody = await response.text().catch(() => '');
      console.error(`[emailService] Falha ao enviar notificação de embarque retroativo (status ${response.status}) para ${params.to}: ${errBody}`);
      return { enviado: false, motivo: `A Resend recusou o envio (HTTP ${response.status}).` };
    }
