...
// Fase 0-4 do pacote de 21/09 (compartilhado pelo usuário) — rotas novas extraídas para
// adminPacote2109.ts (arquivo próprio só por tamanho — este arquivo já estava muito grande
// para publicar de uma vez; nenhuma mudança de comportamento, mesmo prefixo /api/v1/admin).
...
router.get('/tenants', requirePermission('clientes', 'ver'), (req: BackofficeAuthenticatedRequest, res) => {
  ...
  if (ator?.actor_type === 'SEGURADORA') {
    ...
    return res.json({ status: 'sucesso', tenants: comStatusCadastro(dbStore.tenants.filter((t) => tenantIds.has(t.id))) });
  }
  ...
  return res.json({ status: 'sucesso', tenants: comStatusCadastro(dbStore.tenants) });
});
...
router.post('/policies', requirePermission('apolices', 'editar'), (req: BackofficeAuthenticatedRequest, res) => {
  const { numero_apolice, ramo, tenant_id, broker_id, co_broker_id, assessoria_id, permitir_inativo_vencido, status, vigencia_inicio, vigencia_fim, lmi, aceita_averbacao_como_destinatario, codigo_interno_seguradora } = req.body;
...
router.put('/policies/:id', requirePermission('apolices', 'editar'), (req: BackofficeAuthenticatedRequest, res) => {
  ...
  const { status, permitir_inativo_vencido, numero_apolice, ramo, insurer_id, broker_id, vigencia_inicio, vigencia_fim, lmi, codigo_interno_seguradora } = req.body;
  ...
  if (codigo_interno_seguradora !== undefined) policy.codigo_interno_seguradora = codigo_interno_seguradora || undefined;
